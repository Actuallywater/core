...
from .const import CONF_KEY_NAME, CONF_REDIS_URL, DOMAIN
...
CONFIG_SCHEMA: vol.Schema = vol.Schema(
    {
        vol.Required(CONF_NAME, default="Get redis key"): selector.TextSelector(),
    }
...
def validate_redis_connection(db_url: str, key: str) -> bool:
    """Validate SQL query."""
    ...
    try:
        ...
        raise redis.exceptions.ConnectionError(error) from error
    except redis.exceptions.TimeoutError as error:
        raise redis.exceptions.TimeoutError(error) from error
...
class RedisConfigFlow(ConfigFlow, domain=DOMAIN):
    ...
    ) -> RedisOptionsFlowHandler:
        """Get the options flow for this handler."""
        return RedisOptionsFlowHandler()
    ...
    async def async_step_user(
        ...
        if user_input is not None:
            db_url = user_input.get(CONF_REDIS_URL)
            key = user_input[CONF_KEY_NAME]

            try:
                # query = validate_sql_select(query)
                # db_url_for_validation = resolve_db_url(self.hass, db_url)
                await self.hass.async_add_executor_job(
                    validate_redis_connection, db_url, key
                )
            except redis.exceptions.ConnectionError:
                errors["db_url"] = "db_url_invalid"
                description_placeholders = {"db_url": "db_url_invalid"}
...
class RedisOptionsFlowHandler(OptionsFlow):
    """Handle redis options."""

    async def async_step_init(
        self, user_input: dict[str, Any] | None = None
    ) -> ConfigFlowResult:
        """Manage redis options."""
        errors = {}
        description_placeholders = {}
        ...
        if user_input is not None:
            ...
            try:
                ...
                if state_class := user_input.get(CONF_STATE_CLASS):
                    options[CONF_STATE_CLASS] = state_class
                # if db_url_for_validation != get_instance(self.hass).db_url:
                #    options[CONF_REDIS_URL] = db_url_for_validation
